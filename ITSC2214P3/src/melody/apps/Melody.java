...
public class Melody {
    ...
    public boolean reverse() {
        if (this.song == null || this.song.isEmpty()) return false;

        StackADT<Note> stack = new LinkedStack<>(); // Temporary stack to reverse the order
        QueueADT<Note> tmpQueue = new LinkedQueue<>();

        // Dequeue all notes and push onto the stack
        while (!this.song.isEmpty()) {
            Note note = this.song.dequeue();
            stack.push(note);
            tmpQueue.enqueue(note); // Enqueue to restore later
        }

        // Pop from the stack to reverse the order back into the song queue
        while (!stack.isEmpty()) {
            this.song.enqueue(stack.pop());
        }

        // Restore the original song queue state
        for (int i = 0; i < tmpQueue.size(); i++) {
            this.song.enqueue(tmpQueue.dequeue());
        }
